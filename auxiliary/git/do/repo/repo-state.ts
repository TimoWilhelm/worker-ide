...
export type HydrationStage = 'plan' | 'scan-deltas' | 'scan-loose' | 'build-segment' | 'done' | 'error';
...
export function asTypedStorage<S>(storage: DurableObjectStorage): TypedStorage<S> {
	...
	const put = <K extends keyof S & string>(key: K, value: S[K]) => storage.put(key as string, value);
