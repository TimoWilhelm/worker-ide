import { text, createLogger, isValidOid } from '@git/common/index';
import migrations from '@git/drizzle/migrations';
import { encodeGitObjectAndDeflate, receivePack, buildPackV2, indexPackOnly, inflateAndParseHeader } from '@git/git/index';
import { doPrefix, r2PackKey } from '@git/keys';
import { DurableObject } from 'cloudflare:workers';
import { migrate } from 'drizzle-orm/durable-sqlite/migrator';

import { getDb as getDatabase, insertPackOids } from './db/index';
import { migrateKvToSql } from './db/migrate';
import { debugState, debugCheckCommit, debugCheckOid } from './debug';
import { commitTree } from './direct-write';
import { enqueueHydrationTask, processHydrationSlice, summarizeHydrationPlan, clearHydrationState } from './hydration/index';
import { handleIdleAndMaintenance } from './maintenance';
import { purgeRepo, removePack } from './pack-operations';
import { getPackLatest, getPacks, getPackOids, getPackOidsBatch } from './packs';
import { getRefs as getReferences, setRefs as setReferences, resolveHead, setHead, getHeadAndRefs as getHeadAndReferences } from './refs';
import { asTypedStorage, objKey as objectKey } from './repo-state';
import { ensureScheduled, scheduleAlarmIfSooner } from './scheduler';
import {
	getObjectStream,
	getObject,
	hasLoose,
	...
	storeObject,
	getObjectsBatch,
	getObjectRefsBatch as getObjectReferencesBatch,
} from './storage';
import { materializeTree, getBlobContent, getBlobContentBatch, getLog, diffTrees, isAncestor } from './tree-read';
import { handleUnpackWork, getUnpackProgress } from './unpack';

import type { RepoStateSchema, Head } from './repo-state';
import type { UnpackProgress } from '@git/common/index';
import type {
	CommitTreeOptions,
	CommitTreeResult,
	TreeEntry as SharedTreeEntry,
	CommitLogEntry,
	TreeDiffEntry,
	EphemeralReference,
 ...
 */
export class RepoDurableObject extends DurableObject<GitWorkerEnvironment> {
	...
	constructor(context: DurableObjectState, environment: GitWorkerEnvironment) {
		super(context, environment);
		context.blockConcurrencyWhile(async () => {
			this.lastAccessMemMs = await context.storage.get('lastAccessMs');
			this.db = getDatabase(context.storage);
	...
	private async ensureAccessAndAlarm(): Promise<void> {
		try {
			...
			try {
				this.logger.warn('touch:schedule:failed', { error: String(error) });
			} catch {}
		}
	}
	...
	public async listRefs(): Promise<{ name: string; oid: string }[]> {
		await this.ensureAccessAndAlarm();
		return await getReferences(this.ctx);
	}

	public async setRefs(references: { name: string; oid: string }[]): Promise<void> {
		await this.ensureAccessAndAlarm();
		await setReferences(this.ctx, references);
	}
	...
	public async getHeadAndRefs(): Promise<{ head: Head; refs: { name: string; oid: string }[] }> {
		await this.ensureAccessAndAlarm();
		return await getHeadAndReferences(this.ctx);
	}
	...
	public async getObjectRefsBatch(oids: string[]): Promise<Map<string, string[]>> {
		await this.ensureAccessAndAlarm();
		return await getObjectReferencesBatch(this.ctx, oids, this.logger);
	}
	...
	private async handleHydrationWork(store: ReturnType<typeof asTypedStorage<RepoStateSchema>>): Promise<boolean> {
		try {
			...
			this.logger.error('alarm:hydration:error', { error: String(error) });
	 ...
	 */
	public async seedMinimalRepo(withPack: boolean = true): Promise<{ commitOid: string; treeOid: string }> {
		await this.ensureAccessAndAlarm();
		const store = asTypedStorage<RepoStateSchema>(this.ctx.storage);
		const database = getDatabase(this.ctx.storage);
		...
		const commitPayload = `tree ${treeOid}\n` + `author ${author}\n` + `committer ${committer}\n` + `\n${message}`;
		const { oid: commitOid, zdata: commitZ } = await encodeGitObjectAndDeflate('commit', new TextEncoder().encode(commitPayload));
		...
		if (withPack) {
			...
			try {
				await insertPackOids(database, packKey, packOids);
			} catch {}

			// Also store objects as loose in DO for direct access
			await store.put(objectKey(treeOid), treeZ);
			await store.put(objectKey(commitOid), commitZ);
		} else {
	...
	// =========================================================================
	// IDE RPC methods — called by the main worker via cross-worker DO binding
	// =========================================================================

	/**
	 * Create a commit from file contents without a git client.
	 * Primary write path for IDE commits.
	 */
	public async commitTree(options: CommitTreeOptions): Promise<CommitTreeResult> {
		await this.ensureAccessAndAlarm();
		return commitTree(this.ctx, this.env, this.prefix(), options);
	}
	 ...
	 */
	public async materializeTree(reference: string): Promise<SharedTreeEntry[]> {
		await this.ensureAccessAndAlarm();
	...
	public async createEphemeralReference(name: string, sourceReference: string): Promise<EphemeralReference> {
		...
		const head = await resolveHead(this.ctx);

		let targetReference = sourceReference;
		if (targetReference === 'HEAD' && head.target) targetReference = head.target;

		const source = references.find((reference) => reference.name === targetReference);
		const oid = source?.oid ?? head.oid;
		if (!oid) throw new Error(`Cannot resolve source ref: ${sourceReference}`);

		const ephemeralName = `refs/ephemeral/${name}`;
		const existingEphemeral = references.find((reference) => reference.name === ephemeralName);

		if (existingEphemeral) {
			// Update existing
			await setReferences(
				this.ctx,
				references.map((reference) => (reference.name === ephemeralName ? { ...reference, oid } : reference)),
			);
		} else {
			await setReferences(this.ctx, [...references, { name: ephemeralName, oid }]);
		}
	...
	public async promoteEphemeralReference(name: string, targetBranch: string): Promise<void> {
		...
		const references = await getReferences(this.ctx);
		const ephemeralName = `refs/ephemeral/${name}`;
		const ephemeral = references.find((reference) => reference.name === ephemeralName);
		if (!ephemeral) throw new Error(`Ephemeral ref not found: ${name}`);

		const branchName = targetBranch.startsWith('refs/heads/') ? targetBranch : `refs/heads/${targetBranch}`;

		// Add or update the target branch, remove the ephemeral ref
		const updatedReferences = references
			.filter((reference) => reference.name !== ephemeralName && reference.name !== branchName)
			.concat({ name: branchName, oid: ephemeral.oid });
	...
	public async listEphemeralReferences(): Promise<EphemeralReference[]> {
		...
		const references = await getReferences(this.ctx);
		return references
			.filter((reference) => reference.name.startsWith('refs/ephemeral/'))
			.map((reference) => ({
				name: reference.name.replace('refs/ephemeral/', ''),
				oid: reference.oid,
			}));
	}
	...
	public async deleteEphemeralReference(name: string): Promise<void> {
		...
		await setReferences(
			this.ctx,
			references.filter((reference) => reference.name !== ephemeralName),
		);
	}
}
