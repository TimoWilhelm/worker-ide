import { getRepoStub } from '@git/common/stub';
...
export async function capabilityAdvertisement(
	...
	let references: { name: string; oid: string }[] = [];
	if (repoId) {
		try {
			const stub = getRepoStub(environment, repoId);
			const data = await stub.listRefs();
			if (Array.isArray(data)) references = data as { name: string; oid: string }[];
		} catch {}
	}
	const caps = ['report-status', 'delete-refs', 'atomic', 'ofs-delta', `agent=git-on-cloudflare/0.1`].join(' ');
	...
	if (references.length > 0) {
		for (const [index, r] of references.entries()) {
			if (index === 0) lines.push(pktLine(`${r.oid} ${r.name}\0${caps}\n`));
