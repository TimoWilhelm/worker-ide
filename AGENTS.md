...
- [ ] You ran `bun run test:react` to run React component tests and it passes with no errors.
...
- `node:fs/promises` is aliased to `worker-fs-mount/fs` at build time — LSP errors for this import are expected.
- Raw imports (e.g., `?raw`) are resolved by Vite at build time — LSP errors are expected.
