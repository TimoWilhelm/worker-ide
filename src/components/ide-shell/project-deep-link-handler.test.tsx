...
describe('ProjectDeepLinkHandler', () => {
	...
	it('opens an agent session deep link once per unique target', () => {
		const view = render(<ProjectDeepLinkHandler projectId="project-1" deepLink={{ kind: 'agent-session', sessionId: 'session-2' }} />);
		...
		view.rerender(<ProjectDeepLinkHandler projectId="project-1" deepLink={{ kind: 'agent-session', sessionId: 'session-2' }} />);
		expect(mocks.applyProjectDeepLink).toHaveBeenCalledTimes(1);

		view.rerender(<ProjectDeepLinkHandler projectId="project-1" deepLink={{ kind: 'agent-session', sessionId: 'session-3' }} />);
