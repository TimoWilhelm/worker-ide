...
// =============================================================================
// Override: /api/auth/organization/list
//
// Bypasses better-auth's internal adapter join for listing organizations.
// After the schema migration (0012) that recreated the organization table,
// better-auth's fallback join resolution returns empty results. This direct
// DB query is a reliable replacement.
// =============================================================================

app.get('/api/auth/organization/list', async (c) => {
...
// =============================================================================
// Override: /api/auth/organization/get-full-organization
//
// Same rationale as the /list override above — bypasses better-auth's broken
// join resolution after the organization table recreation in migration 0012.
// =============================================================================
...
app.get('/api/auth/organization/get-full-organization', async (c) => {
	...
	if (import.meta.env.DEV) {
		...
		if (!result) return c.json({ error: 'Unauthorized' }, 401);
		userId = result.user.id;
		activeOrganizationId = result.session.activeOrganizationId ?? undefined;
	} else {
		const auth = createAuth(
			{
				DB: c.env.DB,
	...
	const organizationId = c.req.query('organizationId') || activeOrganizationId;
	// eslint-disable-next-line unicorn/no-null -- better-auth client expects null
	if (!organizationId) return c.json(null);
	...
	const organizations = await database
		...
		.limit(1);
	// eslint-disable-next-line unicorn/no-null -- better-auth client expects null
	if (organizations.length === 0) return c.json(null);

	// Verify the user is a member
	const membershipCheck = await database
		.select({ id: authSchema.member.id })
		.from(authSchema.member)
		.where(and(eq(authSchema.member.organizationId, organizationId), eq(authSchema.member.userId, userId)))
		.limit(1);
	// eslint-disable-next-line unicorn/no-null -- better-auth client expects null
	if (membershipCheck.length === 0) return c.json(null);
