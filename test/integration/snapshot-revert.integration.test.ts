...
async function createProject(): Promise<string> {
	const response = await authedFetchWithRetry(`${BASE_URL}/api/new-project`, {
		method: 'POST',
	...
	await authedFetchWithRetry(`${BASE_URL}/p/${result.projectId}/api/files`);
...
/**
 * Create a snapshot through the real internal snapshot-manager flow.
 *
 * Hidden internal paths (`.agent/snapshots/*`) are not writable via the public
 * file API, so seeding goes through the dev-only `/api/__test/seed-snapshot`
 * endpoint, which invokes the same `initSnapshot` + `addFileToSnapshot`
 * functions the agent uses during a code turn. The snapshot id is generated by
 * the real flow and returned to the caller.
 */
async function createSnapshot(
	...
	const response = await authedFetchWithRetry(`${BASE_URL}/p/${projectId}/api/__test/seed-snapshot`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
...
/**
 * Each describe block uses its own isolated project (to avoid file-path
 * collisions between blocks). `/api/new-project` does real work (template
 * materialization + several D1 queries) and takes a few seconds each, so
 * creating them serially in per-block `beforeAll` hooks dominates suite time.
 *
 * Instead, every project creation is kicked off concurrently in the top-level
 * `beforeAll` and stored as a pending promise. Each block then simply awaits
 * its named promise — projects provision in the background, overlapping with
 * earlier blocks' tests.
 */
type ProjectKey =
	| 'list'
	| 'revert'
...
describe('Snapshot & Revert Integration Tests', () => {
	...
	let projectPromises: Record<ProjectKey, Promise<string>>;
	...
	beforeAll(async () => {
		...
		// Kick off all project creations concurrently (each is independent).
		projectPromises = {
			list: createProject(),
			revert: createProject(),
			fileRevert: createProject(),
			cascade: createProject(),
			dedup: createProject(),
			missing: createProject(),
			validation: createProject(),
			pending: createProject(),
			fileContent: createProject(),
			edge: createProject(),
			multiSession: createProject(),
			disjoint: createProject(),
			sessionId: createProject(),
			idempotent: createProject(),
			large: createProject(),
			order: createProject(),
		};

		// The first block uses projectId directly (no beforeAll of its own).
		projectId = await projectPromises.list;
	}, 120_000);
	...
	describe('Snapshot Listing & Detail', () => {
		...
		it('lists a manually created snapshot', async () => {
			listSnapshotId = await createSnapshot(projectId, {
			...
			const response = await authedFetch(`${BASE_URL}/p/${projectId}/api/snapshots`);
			const result: { snapshots: SnapshotSummary[] } = await response.json();
			const snapshot = result.snapshots.find((s) => s.id === listSnapshotId);
		...
		it('GET /api/snapshot/:id returns full metadata', async () => {
			const response = await authedFetch(`${BASE_URL}/p/${projectId}/api/snapshot/${listSnapshotId}`);
			expect(response.ok).toBe(true);
			const result: { snapshot: SnapshotMetadata } = await response.json();
			expect(result.snapshot.id).toBe(listSnapshotId);
	...
	describe('Single Snapshot Revert', () => {
		...
		beforeAll(async () => {
			revertProjectId = await projectPromises.revert;
		});
		...
		it('reverts a created file (deletes it)', async () => {
			...
			const snapshotId = await createSnapshot(revertProjectId, {
				changes: [{ path: '/src/new-file.ts', action: 'create' }],
			});

			// Revert
			const response = await authedFetch(`${BASE_URL}/p/${revertProjectId}/api/snapshot/${snapshotId}/revert`, {
				method: 'POST',
			});
			expect(response.ok).toBe(true);
	...
	describe('Single File Revert', () => {
		...
		let fileRevertSnapshotId: string;

		beforeAll(async () => {
			fileRevertProjectId = await projectPromises.fileRevert;
		});
		...
		it('reverts a single file from a multi-file snapshot', async () => {
			...
			fileRevertSnapshotId = await createSnapshot(fileRevertProjectId, {
				changes: [
			...
			const response = await authedFetch(`${BASE_URL}/p/${fileRevertProjectId}/api/snapshot/revert-file`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ path: '/src/revert-me.ts', snapshotId: fileRevertSnapshotId }),
			});
			expect(response.ok).toBe(true);
		...
		it('returns 404 for a file not in the snapshot', async () => {
			const response = await authedFetch(`${BASE_URL}/p/${fileRevertProjectId}/api/snapshot/revert-file`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ path: '/src/not-in-snapshot.ts', snapshotId: fileRevertSnapshotId }),
			});
			expect(response.status).toBe(404);
		});
	...
	describe('Cascade Revert — Happy Path', () => {
		...
		beforeAll(async () => {
			cascadeProjectId = await projectPromises.cascade;
		});
		...
		it('reverts a single snapshot via cascade (equivalent to single revert)', async () => {
			...
			const snapshotId = await createSnapshot(cascadeProjectId, {
			...
			const response = await authedFetch(`${BASE_URL}/p/${cascadeProjectId}/api/snapshots/revert-cascade`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ snapshotIds: [snapshotId] }),
			});
			expect(response.ok).toBe(true);
		...
		it('cascade-reverts two snapshots in reverse chronological order', async () => {
			...
			const turn1SnapshotId = await createSnapshot(cascadeProjectId, {
			...
			const turn2SnapshotId = await createSnapshot(cascadeProjectId, {
				sessionId: 'sess-cascade',
				changes: [
			...
			// Cascade revert: newest first (turn 2, then turn 1)
			const response = await authedFetch(`${BASE_URL}/p/${cascadeProjectId}/api/snapshots/revert-cascade`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ snapshotIds: [turn2SnapshotId, turn1SnapshotId] }),
			});
			expect(response.ok).toBe(true);
	...
	describe('Cascade Revert — File Deduplication', () => {
		...
		beforeAll(async () => {
			dedupProjectId = await projectPromises.dedup;
		});
		...
		it('uses the earliest snapshot backup when the same file appears in multiple snapshots', async () => {
			...
			const turn1SnapshotId = await createSnapshot(dedupProjectId, {
			...
			const turn2SnapshotId = await createSnapshot(dedupProjectId, {
			...
			const turn3SnapshotId = await createSnapshot(dedupProjectId, {
			...
			const response = await authedFetch(`${BASE_URL}/p/${dedupProjectId}/api/snapshots/revert-cascade`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ snapshotIds: [turn3SnapshotId, turn2SnapshotId, turn1SnapshotId] }),
			});
			expect(response.ok).toBe(true);
			...
			expect(result.reverted[0].snapshotId).toBe(turn1SnapshotId);
	...
	describe('Cascade Revert — Missing Snapshots', () => {
		...
		beforeAll(async () => {
			missingProjectId = await projectPromises.missing;
		});
		...
		it('reports missing snapshots but still reverts available ones', async () => {
			...
			const snapshotId = await createSnapshot(missingProjectId, {
			...
			const response = await authedFetch(`${BASE_URL}/p/${missingProjectId}/api/snapshots/revert-cascade`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ snapshotIds: ['deadbeef', snapshotId] }),
			});
			expect(response.ok).toBe(true);
	...
	describe('Cascade Revert — Validation', () => {
		...
		beforeAll(async () => {
			validationProjectId = await projectPromises.validation;
		});
	...
	describe('Pending Changes Coordination', () => {
		...
		beforeAll(async () => {
			pendingProjectId = await projectPromises.pending;
		});
		...
		it('pending changes survive independently of snapshot revert', async () => {
			...
			// Revert the snapshot for from-snap-a.ts (file-level only)
			await writeFile(pendingProjectId, '/src/from-snap-a.ts', 'modified-a');
			const snapshotId = await createSnapshot(pendingProjectId, {
				changes: [{ path: '/src/from-snap-a.ts', action: 'edit', beforeContent: 'original-a' }],
			});
			const response = await authedFetch(`${BASE_URL}/p/${pendingProjectId}/api/snapshot/${snapshotId}/revert`, {
				method: 'POST',
			});
			expect(response.ok).toBe(true);
	...
	describe('Snapshot File Content API', () => {
		...
		let fileContentSnapshotId: string;

		beforeAll(async () => {
			fileContentProjectId = await projectPromises.fileContent;

			// Create a snapshot with backed up content
			fileContentSnapshotId = await createSnapshot(fileContentProjectId, {
				changes: [
		...
		it('returns file content from a snapshot', async () => {
			const response = await authedFetch(
				`${BASE_URL}/p/${fileContentProjectId}/api/snapshot/${fileContentSnapshotId}/file?path=${encodeURIComponent('/src/backed-up.ts')}`,
			);
			expect(response.ok).toBe(true);
		...
		it('returns undefined content for created files (no before content)', async () => {
			const response = await authedFetch(
				`${BASE_URL}/p/${fileContentProjectId}/api/snapshot/${fileContentSnapshotId}/file?path=${encodeURIComponent('/src/was-created.ts')}`,
			);
			expect(response.ok).toBe(true);
		...
		it('returns 404 for a file not in the snapshot', async () => {
			const response = await authedFetch(
				`${BASE_URL}/p/${fileContentProjectId}/api/snapshot/${fileContentSnapshotId}/file?path=${encodeURIComponent('/src/not-here.ts')}`,
			);
			expect(response.status).toBe(404);
		});
	...
	describe('Edge Case: Revert of Already-Deleted File', () => {
		...
		beforeAll(async () => {
			edgeProjectId = await projectPromises.edge;
		});
		...
		it('reverting a created file that was already manually deleted is a no-op', async () => {
			...
			const snapshotId = await createSnapshot(edgeProjectId, {
				changes: [{ path: '/src/already-gone.ts', action: 'create' }],
			});
			// File doesn't exist on disk (user deleted it or it was never truly written)

			const response = await authedFetch(`${BASE_URL}/p/${edgeProjectId}/api/snapshot/${snapshotId}/revert`, {
				method: 'POST',
			});
	...
	describe('Edge Case: Multiple Sessions Touching Same File', () => {
		...
		beforeAll(async () => {
			multiSessionProjectId = await projectPromises.multiSession;
		});
		...
		it('cascade revert only reverts snapshots it is given, leaving other snapshots intact', async () => {
			...
			const session1SnapshotId = await createSnapshot(multiSessionProjectId, {
				sessionId: 'session-1',
			...
			const session2SnapshotId = await createSnapshot(multiSessionProjectId, {
				sessionId: 'session-2',
			...
			const response = await authedFetch(`${BASE_URL}/p/${multiSessionProjectId}/api/snapshots/revert-cascade`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ snapshotIds: [session2SnapshotId] }),
			});
			expect(response.ok).toBe(true);
			...
			const snap1 = await authedFetch(`${BASE_URL}/p/${multiSessionProjectId}/api/snapshot/${session1SnapshotId}`);
		...
		it('cascade revert of session 1 restores to v0 even though session 2 also modified the file', async () => {
			...
			const turn1SnapshotId = await createSnapshot(multiSessionProjectId, {
				sessionId: 'session-1',
				changes: [{ path: '/src/shared.ts', action: 'edit', beforeContent: 'v0' }],
			});
			await writeFile(multiSessionProjectId, '/src/shared.ts', 'v1');
			const turn2SnapshotId = await createSnapshot(multiSessionProjectId, {
				sessionId: 'session-1',
	...
	describe('Edge Case: Disjoint Files Across Snapshots', () => {
		...
		beforeAll(async () => {
			disjointProjectId = await projectPromises.disjoint;
		});

		it('reverts all files from all snapshots when they do not overlap', async () => {
			// Turn 1: creates file-a.ts
			const turn1SnapshotId = await createSnapshot(disjointProjectId, {
			...
			const turn2SnapshotId = await createSnapshot(disjointProjectId, {
			...
			const turn3SnapshotId = await createSnapshot(disjointProjectId, {
	...
	describe('Snapshot sessionId field', () => {
		...
		beforeAll(async () => {
			sessionIdProjectId = await projectPromises.sessionId;
		});

		it('snapshot metadata includes sessionId when provided', async () => {
			const snapshotId = await createSnapshot(sessionIdProjectId, {
				label: 'Scoped snapshot',
				sessionId: 'sess-abc123',
				changes: [{ path: '/src/test.ts', action: 'create' }],
			});

			const response = await authedFetch(`${BASE_URL}/p/${sessionIdProjectId}/api/snapshot/${snapshotId}`);
			expect(response.ok).toBe(true);
			const result: { snapshot: SnapshotMetadata } = await response.json();
		...
		it('snapshot metadata works without sessionId (backwards compatibility)', async () => {
			// Create a snapshot without a sessionId (e.g. a project-level snapshot)
			const snapshotId = await createSnapshot(sessionIdProjectId, {
	...
	describe('Edge Case: Cascade Revert Idempotency', () => {
		...
		beforeAll(async () => {
			idempotentProjectId = await projectPromises.idempotent;
		});
		...
		it('reverting the same cascade twice does not cause errors', async () => {
			...
			const snapshotId = await createSnapshot(idempotentProjectId, {
	...
	describe('Edge Case: Large Cascade', () => {
		...
		beforeAll(async () => {
			largeProjectId = await projectPromises.large;
		});
		...
		it('handles cascading 10 snapshots with unique files', async () => {
			...
			for (let index = 0; index < 10; index++) {
				...
				const snapshotId = await createSnapshot(largeProjectId, {
	...
	describe('Edge Case: Snapshot Listing Order', () => {
		...
		beforeAll(async () => {
			orderProjectId = await projectPromises.order;
		});

		it('snapshots are listed newest-first', async () => {
			const olderSnapshotId = await createSnapshot(orderProjectId, {
				label: 'Older',
			...
			const newerSnapshotId = await createSnapshot(orderProjectId, {
				label: 'Newer',
			...
			const ourSnapshots = result.snapshots.filter((s) => s.id === olderSnapshotId || s.id === newerSnapshotId);
			expect(ourSnapshots).toHaveLength(2);
			// Newer should be first
			expect(ourSnapshots[0].id).toBe(newerSnapshotId);
			expect(ourSnapshots[1].id).toBe(olderSnapshotId);
		});
	});
});
