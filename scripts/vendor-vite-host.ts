...
function aliasPlugin(): EsbuildPlugin {
	...
	const reactPluginStub = path.join(VITE_HOST, 'runtime/react-plugin-stub.ts');
	...
	const exact = new Map<string, string>([
		...
		// The host transpiles JSX with esbuild and implements React Fast Refresh in
		// its dev module server, so @vitejs/plugin-react's Babel transform is dead
		// weight. Stub it to keep Babel (~40 MB of import-time heap) out of the
		// bundle for both preview and deploy builds.
		['@vitejs/plugin-react', reactPluginStub],
	]);
...
/**
 * Compress a `path -> source` map to `path -> gzip(source) as base64`. The host
 * seeds these as read-only {@link VendoredLayer}s and decompresses each file
 * lazily on first read, so an isolate holds the source compressed (~4-5×
 * smaller) plus only the handful of files a given build actually touches —
 * instead of all ~6.7 MB of React/RSC/runtime source resident as strings.
 */
function compressFileMap(files: Record<string, string>): Record<string, string> {
	const compressed: Record<string, string> = {};
	for (const [key, contents] of Object.entries(files)) {
		compressed[key] = gzipSync(Buffer.from(contents, 'utf8')).toString('base64');
	}
	return compressed;
}
...
function writeVinextRuntimeMap(): void {
	...
	writeCompressedFileMap(RUNTIME_FILE, files, 'vinext-runtime');
}

/**
 * Write a vendored file map as `path -> gzip(source) base64`, plus a colocated
 * `.d.ts`. The runtime seeds it as a {@link VendoredLayer} and decompresses each
 * file lazily, so the source lives compressed in the build isolate.
 */
function writeCompressedFileMap(outputFile: string, files: Record<string, string>, label: string): void {
	const banner = '/* AUTO-GENERATED by scripts/vendor-vite-host.ts — do not edit. */\n';
	const compressed = compressFileMap(files);
	writeFileSync(outputFile, `${banner}export default ${JSON.stringify(compressed)};\n`);
	// Colocated declaration so TypeScript (and the typed ESLint parser) type the
	// import without loading the multi-MB data file into the program. Values are
	// gzip+base64; the runtime decodes them via VendoredLayer.fromCompressedRecord.
	writeFileSync(outputFile.replace(/\.js$/, '.d.ts'), `${banner}declare const files: Record<string, string>;\nexport default files;\n`);
	const rawKb = Math.round(Object.values(files).reduce((sum, content) => sum + content.length, 0) / 1024);
	const gzipKb = Math.round(Object.values(compressed).reduce((sum, content) => sum + content.length, 0) / 1024);
	console.log(
		`[vendor-vite-host] wrote ${path.relative(ROOT, outputFile)} (${label}: ${Object.keys(files).length} files, ${rawKb} KB raw → ${gzipKb} KB gzip+base64)`,
	);
}
...
function writeNodeModulesPackages(): void {
	...
	writeCompressedFileMap(NODE_MODULES_FILE, baseFiles, 'base');
	writeCompressedFileMap(NODE_MODULES_DEVELOPMENT_FILE, developmentFiles, 'development-only');
}
