...
function writeNodeModulesPackages(): void {
	...
	for (const packageName of VENDORED_NODE_MODULES) {
		collectPackageFiles(packageName, files);
	}
	const banner = '/* AUTO-GENERATED by scripts/vendor-vite-host.ts — do not edit. */\n';
	writeFileSync(NODE_MODULES_FILE, `${banner}export default ${JSON.stringify(files)};\n`);
	writeFileSync(
		NODE_MODULES_FILE.replace(/\.js$/, '.d.ts'),
		`${banner}declare const files: Record<string, string>;\nexport default files;\n`,
	);
	const bytes = Object.values(files).reduce((sum, content) => sum + content.length, 0);

	console.log(
		`[vendor-vite-host] wrote ${path.relative(ROOT, NODE_MODULES_FILE)} (${Object.keys(files).length} files, ${Math.round(bytes / 1024)} KB)`,
	);
}
